...
OUT_FOLDER = configs.OUT_ALL_NGRAMS_FOLDER
        ...
        for word_filename, (boxes, transcriptions) in all_lines.items():
            ...
            for box, trans in zip(boxes, transcriptions):
                if box[0]==box[1]:
                    ...
                    if not os.path.exists(os.path.join(dst_folder, trans)):
                        os.mkdir(os.path.join(dst_folder, trans))
                    n_elem_infolder = len(os.listdir(os.path.join(dst_folder, trans)))

                    new_filename = str(n_elem_infolder).zfill(configs.MAX_LEN_NGRAM_NAME) +"."+ word_filename.split(".")[-1]

                    word_img = line_img[: , box[0]:box[1]]
                    cv2.imwrite(os.path.join(dst_folder, trans, new_filename), word_img)
